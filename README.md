...
SwiftUIを使用  
キーワードを入力し、検索ボタンをタップすると複数のサイトでの検索結果をタブで表示。
履歴機能で過去に検索したキーワードを再検索できます。
UserDefaultsに20件まで検索履歴を格納。

## 作成理由
買い物などで気になった商品の中古の相場を調べるため、各フリマサイトのアプリを立ち上げて商品名をコピペするのは非効率なので、キーワードを入力したら一括で検索し表示結果をタブで切り替えられるアプリを作成。

## 工夫した点
- 検索履歴を過去20件までUserDefaultsに保存、検索キーワードが同じものは保存しないよう実装。
- ProgressViewを使用し、読み込みがユーザーに視覚的にわかるよう実装

## ユーザーからのレビュー  
ユーザーからのレビューで特定のサイトのヘッダーを消してほしい､履歴を削除できるようにしてほしいと要望あり以下プルリクで対応し､アップデートした｡  
WKWebViewのevaluateJavaScriptを使用しブラウザのヘッダー削除 https://github.com/kabikira/FleaMarketSearch/pull/8  
